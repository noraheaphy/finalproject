...
anacampsTree <- drop.tip(anacampsTree, c(21,16,17,18,13,22,11,10,9,8)) 
# monophyletic duplicates
...
node_values = c(anacampsData$MAT, rep(NA, n_tips-1))
...
node_values = c(anacampsData[,21], mat_ancestral$ace)
...
# nodes. Since we don't have values for the internal nodes, we will pad them 
# with NA (Not Available) for now.
node_values = c(anacampsData[,21], mat_ancestral_pic$ace )
...
D_methods %>%
  ...
  geom_point() +
  geom_text(aes(label = node), color = "red", vjust = 0, nudge_y = 0.05, size = 3) +
  geom_abline(slope = 1, intercept = 0, color = "blue") +
  labs(title = "Anacampserotaceae", x = "Restricted Maximum Likelihood", 
       y = "Phylogenetic Independent Contrasts")
...
node_values = c(rep(NA, nrow(anacampsData)), interval_width)
...
anacampsData <- anacampsData %>% select(MAT)
